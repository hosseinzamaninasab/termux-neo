...
beta_run_matrix() {
    ...
    [[ "$expected_version" == "1.0.0-rc.1" ]] ||
        beta_fail "field candidate is not 1.0.0-rc.1"
...
beta_write_report() {
    ...
    {
        printf '# Release Candidate Field Report\n\n'
        printf 'This report was generated by the strict Task 31 field gate. '
...
if [[ "$MATRIX_MODE" == "record" ]]; then
    beta_write_report
    printf 'PASS: strict release candidate field record created\n'
    printf 'report: %s\n' "$REPORT_OUTPUT"
else
    printf 'PASS: portable release candidate field matrix (10 scenarios)\n'
fi
