...
beta_error() {
    printf 'termux-neo beta: %s\n' \
        "${1-stable field test failed}" >&2
}

beta_fail() {
    beta_error "${1-stable field test failed}"
    exit 1
}
...
beta_run_matrix() {
    ...
    IFS= read -r expected_version < "$MATRIX_SOURCE/VERSION" ||
        beta_fail "stable VERSION could not be read"
    [[ "$expected_version" == "1.0.0" ]] ||
        beta_fail "field release is not 1.0.0"
...
beta_prepare_artifact() {
    ...
    [[ -d "$package_root" && ! -L "$package_root" ]] ||
        beta_fail "extracted stable artifact root is invalid"
    (
        cd "$package_root"
...
beta_write_report() {
    ...
    {
        printf '# Stable Release Field Report\n\n'
        printf 'This report was generated by the strict Task 32 field gate. '
        ...
        printf '| Clean install | PASS | Extracted stable artifact |\n'
        ...
        printf '## Defect and release gate\n\n'
        printf -- '- Open critical security defects: `0`\n'
        printf -- '- Open high-severity defects: `0`\n'
        printf -- '- Open release-blocking defects: `0`\n'
        printf -- '- Feature freeze: COMPLETE\n'
        printf -- '- Task 32 result: PASS\n\n'
...
grep -Fqx 'Open high-severity defects: 0' \
    "$SOURCE_ROOT/docs/beta-issues.md" ||
    beta_fail "high-severity defect gate is not zero"
grep -Fqx 'Open release-blocking defects: 0' \
    "$SOURCE_ROOT/docs/beta-issues.md" ||
    beta_fail "release-blocking defect gate is not zero"
grep -Fqx 'COMPLETE — 1.0.0' \
    "$SOURCE_ROOT/docs/feature-freeze.md" ||
    beta_fail "stable feature-freeze checkpoint is incomplete"
...
if [[ "$MATRIX_MODE" == "record" ]]; then
    beta_write_report
    printf 'PASS: strict stable release field record created\n'
    printf 'report: %s\n' "$REPORT_OUTPUT"
else
    printf 'PASS: portable stable release field matrix (10 scenarios)\n'
fi
